...
ypitch=0.3/np.cos(sense_angle_deg/360.*np.pi*2) # sense/field wire pitch
...
sim_vol_ysize=3./np.cos(sense_angle_deg/360.*np.pi*2) # along the cathode wires
...
## section header
print "*****************CELL***********************"
print "* generating MDC II geometry at {:} V (field) / {:} V (cathodes)".format(v_field,v_cath)
print "&CELL"
print "solids"
...
for y in np.arange(0,sim_vol_ysize/2.,ypitch*2) :
  for z in np.arange(0,sim_vol_zsize/2.,zpitch*2) :
    ...
    print "      direction "+sense_direction+" ..."
...
for y in np.arange(0,sim_vol_ysize/2.,ypitch*2) :
  for z in np.arange(0,sim_vol_zsize/2.,zpitch*2) :
    ...
    if ( (z/zpitch)%4 == 0):
      y_bar = y+ypitch
    print "wire  centre {:} {:} {:} ...".format(x,y_bar,z)
    print "      direction "+field_direction+" ..."
    print "      radius {:} ... ".format(sense_wire_rad)
    print "      half-length {:} ...".format(sim_vol_xsize/2.)
