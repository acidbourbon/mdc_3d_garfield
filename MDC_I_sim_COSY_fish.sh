...
# )
root -l 'track_generators/gen_cosy_tracks_fish.C("./tracks/input_tracks.txt","20000","1.6","0.1","0.25")' -q
...
if [ $root_drift_times ]; then
  ...
  for i in *; do
    perl -pi -e "s/^/$i  /g" $i
    grep -P "Hit [ST]" $i |  perl -pi -e "s/unavailable unavailable unavailable  Hit [ST] solid//g" >> track_drift_line_data.txt
  done
  cd $here
#   xterm -e root -l 'ascii_to_ttree.C("tracks/track_drift_line_data.txt")' &
  root -l 'ascii_to_ttree_fish.C("tracks/track_drift_line_data.txt")'
fi
